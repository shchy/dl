...
namespace dl.DL
{
    public class PoolingLayer : ILayer
    {
        ...
        public PoolingLayer(ILayer before, int beforeLayerWidth, int chSize, int poolingSize, int stride)
        {
            ...
            var height = (before.Nodes.Count() / beforeLayerWidth) / chSize;
            ...
            this.Nodes = (
                ...
                from y in Enumerable.Range(0, height).Where(i => i % stride == 0)
                from x in Enumerable.Range(0, beforeLayerWidth).Where(i => i % stride == 0)
                let links = filterNodes.MakeLink(beforeLayerWidth, poolingSize, x, y, (wx, wy) => Weight.Make(1.0)).ToArray()
